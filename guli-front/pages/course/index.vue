        ...
        <div>
          <div class="paging">
            ...
            <a
              ...
              href="#"
              title="后一页"
              @click.prevent="gotoPage(data.hasNext?data.current+1:data.current)">&gt;</a>
  ...
  methods: {
    ...
    initCourseFirst() {
      courseApi.getCourseList(1, 8, this.searchObj).then((resp) => {
        console.log(this.searchObj.subjectParentId)
    ...
    searchOne(subjectParentId, index) {
      //4.3 把传递index值赋值给oneIndex,为了active样式生效
      this.oneIndex = index
      this.twoIndex = -1
      this.searchObj.subjectId = ""
      this.subSubjectList = []
      //样式结束
      //4.2 把一级分类点击的id值，赋值给searchObj
      this.searchObj.subjectParentId = subjectParentId;
      //点击某个一级分类进行条件查询
      this.gotoPage(1);
      //4.2
      //4.1 拿着点击的一级分类id 和 所有一级分类id进行比较
    ...
    //5. 点击全部查询全部数据
    allCourse() {
      //将定义过的subjectParentId设置为undefined
      this.searchObj.subjectParentId = undefined
      //清空二级查询
